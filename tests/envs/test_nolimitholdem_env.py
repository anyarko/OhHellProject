...
class TestNolimitholdemEnv(unittest.TestCase):
    ...
    def test_step_back(self):
        env = rlohhell.make('no-limit-holdem', config={'allow_step_back':True})
    ...
    def test_multiplayers(self):
        env = rlohhell.make('no-limit-holdem', config={'game_num_players':5})
        num_players = env.game.get_num_players()
        self.assertEqual(num_players, 5)

    def test_config_chips(self):
        env = rlohhell.make('no-limit-holdem', config={'game_num_players':5, 'chips_for_each':[100, 200, 400, 600, 900]})
        env.game.init_game()
